...
<%
    ...
    Perfil p = new Perfil();
    ArrayList<Menu> menusFora = new ArrayList<Menu>();
    try {
        int id = Integer.parseInt(request.getParameter("id"));
        p.carregarPorId(id);
        menusFora = p.menusNaoPerfil(id);
    } catch (Exception e) {
        out.print("Erro: " + e);
    }
                        ...
                        <div id="divTableList" class="divTableList col-xs-10 col-xs-offset-1">
                            <center>
                                <form method="post" action="gerenciar_menu_perfil.do">
                                    ...
                                    <h4>ID:<%=p.getId()%> PERFIL: <%=p.getNome()%></h4><br/>
                                    <p>Menu:&nbsp;&nbsp;</p><select name="id_menu" size="1" style="display: inline-block;">
                                        <option value="">Selecione...</option>   
                                        <%
                                            for (Menu m : menusFora) {
                                        %>
                                ...
                                <table class="tableList table table-responsive">
                                    ...
                                    <tr>
                                        <td><%=m.getId()%></td>
                                        <td><%=m.getNome()%></td>
                                        <td><a href="gerenciar_menu_perfil.do?op=desvincular&id_menu=<%=m.getId()%>&id_perfil=<%=p.getId()%>" title="Excluir"><i class="fa fa-times" aria-hidden="true"></i></a></td>&nbsp;&nbsp;
                                    </tr>
                                    <%
                                        }
                                    %>
